...
function findGifs($newGifs, option) {
	...
	const images = Array.from(
		container.querySelectorAll(
			`:is(img[src*=".gif"]${additionalImages}):not([src*="gifa11y-ignore"], [data-gifa11y-state], .gifa11y-ignore${exclusions})`,
		),
	);
...
function toggleAll(newState = "detect") {
	...
	if (state === "detect") {
		state =
			html.getAttribute("data-gifa11y-all") === "paused" ? "playing" : "paused";
...
function generateButtons(gif, option) {
  ...
  <div class="play" aria-hidden="true" style="display:${playDisplay}"></div>`;
	...
	const location =
		image.closest('a, button, [role="link"], [role="button"]') || image;
	...
	constructor(options) {
		...
		this.initialize = () => {
			...
			const checkRunPrevent = () => {
				...
				return gifa11yOff.trim().length > 0
					? document.querySelector(gifa11yOff)
					: false;
			};
